...
class PepResponse
{
    public:
		...
		stdext::hash_map<string,Obligation>* getObligations();
		...
		PepResponse* createPepResponse(vector<AzResourceActionAssociation>* actionResourceAssociations,
										PepRequest* inPepRequest,
										bool queryAllowed) 
		{	
			PepResponse* pResponse = new PepResponse();
		...
		vector<AzResourceActionAssociation,allocator<AzResourceActionAssociation>>::iterator azRAAIterator;

	   	AzResult* currentResult;

	private:
		stdext::hash_map<string,Obligation> obligations;
